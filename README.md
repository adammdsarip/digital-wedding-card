...
photograph is beautiful as **art direction reference**
— it set the palette (ivory paper, deep crimson wax, muted gold), the
proportions, and the overall stationery-photography mood — but a flat photo
...
- **`.envelope__flap`** is a clipped triangle that rotates on `rotateX` around
  ...
  believable dark paper tone once it swings past 90°, not a mirrored front.
- **`.wax-seal`** is a single circular gradient at rest; on tap it presses,
  jitters, then reveals two clipped half-copies of itself that rotate apart
  and fall while small wax-fragment fragments (`<i>` blobs) scatter and fade
  — no sprite sheet or video required.
...
1. **Idle** — closed envelope, ambient light drift, breathing floor shadow.
...
6. **The title reveals** — names, ampersand, rule line, date — staggered.
